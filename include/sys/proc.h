...
// typedef int elem_t;
typedef struct proc *elem_t;

// #define Q_MAX 20
#define Q_MAX (NR_PROCS + NR_TASKS)
#define Q_MEM_SIZE ((Q_MAX)+1)
#define INDEX(x) (((x) + (Q_MEM_SIZE)) % (Q_MEM_SIZE))
// #define ELEMS(index) elems[INDEX(index)]

/**
 * \brief Queue;
 * rear points to the space after last elem,
 * front points to the first elem.
 * rear == front -> empty
 * rear + 1 == front -> full 
 * [Q->front, Q->rear)
 **/
typedef struct _queue
{
    int front;
    int rear;
    elem_t elems[Q_MEM_SIZE];
} queue;

#define FRONT(Q) ((Q).elems[(Q).front])
#define EMPTY(Q) ((Q).rear == (Q).front)
...
/**
 \return position of inserted x in Q; -1 if Q is full
**/
...
int runnable(queue *Q);
void wash(queue Q[], queue *Q_block, queue *Q_new);
// void wash(queue Q[], queue *Q_block);
...
extern queue Q[Q_N];
extern int tick_of[Q_N];
extern int current; // index of current Q
#endif