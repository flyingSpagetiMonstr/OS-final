...
// disp_str("INIT");
	...
	{
		if (i >= NR_TASKS + NR_NATIVE_PROCS) // 0 ~ 8
		{
			p->p_flags = FREE_SLOT;
			continue;
		}
...
// ############################
	...
	{
		enqueue(&Q[0], proc_table + index);
		// if((proc_table + index)->p_flags != 0) disp_color_str(".", RED);
	}
	p_proc_ready = dequeue(&Q[0]);
...
void TestA()
{
	for(;;)
	{
		// kill(10); // 10: pid of second shell
		// delay(0xff);
	}
}
