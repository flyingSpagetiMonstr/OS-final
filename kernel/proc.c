...
int tick_of[Q_N] = {2, 4, 8};
int current = 0; // index of current Q
...
PUBLIC void schedule()
{
	...
	int next = (current != Q_N - 1);

	// p_proc_ready->ticks maybe 0, and still get enqueued
	int ret = 0;
...
PUBLIC void inform_int(int task_nr)
{
	...
	{
// disp_color_str(" :inform_int-if: ", RED);
...
// disp_color_str(" :inform_int-if_end: ", RED);
	}
	else {
		p->has_int_msg = 1;
	}
...
/**
 \return a queue with front = rear = 0
**/
queue newqueue(void)
{
    queue Q = {0, 0};
...
/**
 \return position of inserted x in Q; -1 if Q is already full
**/
int enqueue(queue *Q, elem_t x)
{
    if (INDEX(Q->rear + 1) == Q->front)
    {
        return -1;
    }
    Q->elems[Q->rear] = x;
    Q->rear = INDEX(Q->rear + 1);
    return INDEX(Q->rear - 1);
}
...
elem_t dequeue(queue *Q)
{
    ...
    Q->front = INDEX(Q->front + 1);
    return Q->elems[INDEX(Q->front - 1)];
}
...
{
    return INDEX(Q->rear - Q->front);
}

void wash(queue Q[], queue *Q_block, queue *Q_new)
{
	queue old[Q_N] = {0};
	queue _old_block = {0}, _old_new = {0};
	queue *old_block = &_old_block, *old_new = &_old_new;
	...
	Q_copy(old_block, Q_block);
	Q_set(Q_block);

	Q_copy(old_new, Q_new);
	Q_set(Q_new);
	...
	while (!EMPTY(*old_block))
	{
		...
		{
			ret = enqueue(&Q[Q_N-1], p);
		}
		assert(ret != -1);
	}
	

	for (int index = 0; index < Q_N; index++)
	{
		while (!EMPTY(old[index]))
		{
			p = dequeue(&old[index]);
			if (p->p_flags == 0)
			{
				ret = enqueue(&Q[index], p);
			}
			else
			{
				if (p->p_flags == FREE_SLOT)
...
void Q_copy(queue *dst, queue *src)
{
	dst->front = src->front;
	dst->rear = src->rear;
	for (int i = 0; i < Q_MEM_SIZE; i++)
	{
